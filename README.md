...
系统自动追踪每条记忆的访问情况，支持重要性评分和生命周期管理：

| 统计字段 | 说明 | 更新时机 |
| :--- | :--- | :--- |
| **access_count** | 累计访问次数 | 每次检索到该记忆时 +1 |
| **last_accessed_at** | 最近访问时间戳 | 每次检索时更新 |

**数据持久化保证：**
- 使用 UPSERT (INSERT ... ON CONFLICT DO UPDATE) 语法确保访问统计不会被覆盖
- 文件重新索引时保留现有 meta 记录，只删除文本变化导致的废弃 chunk
- 访问统计数据在重要性评分、记忆合并和生命周期管理中发挥关键作用
